...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "__rejseafregning" because\n';
  ...
  $wnd.__gwt_activeModules['__rejseafregning'] = {
    'moduleName' : '__rejseafregning',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/__rejseafregning.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['__rejseafregning'].superdevmode = true;

  var expectedSuffix = '/__rejseafregning.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + __rejseafregning + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:__rejseafregning';
  var devModeSessionKey = '__gwtDevModeSession:__rejseafregning';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/__rejseafregning/__rejseafregning.nocache.js';
  ...
  $wnd.__gwt_activeModules['__rejseafregning'].canRedirect = true;
  $wnd.__gwt_activeModules['__rejseafregning'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING __rejseafregning';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/__rejseafregning?user.agent=' + ua + '&_callback=' + callback;
